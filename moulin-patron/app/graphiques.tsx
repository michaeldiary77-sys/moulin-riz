...
import { AppBar, IconButton } from '@/components/ui/material';
...
import { ajouterJours, dateLocaleIso } from '@/lib/utils/periode';

export default function GraphiquesScreen() {
  const { debut, fin, libelle } = useLocalSearchParams<{
    debut?: string;
    fin?: string;
    libelle?: string;
  }>();
  ...
  const kgParJour = useMemo(() => {
    ...
    for (const c of clients) {
      map.set(c.date, (map.get(c.date) ?? 0) + c.kg);
    }
    return [...map.entries()].map(([label, valeur]) => ({
      label: label.slice(8),
      valeur,
    }));
  }, [clients, d0, d1]);
